...
import com.ceiba.demo.infrastructure.adapter.application.springapp.Application;
...
public class PersonRestControllerTest {
	...
	public void savePersonYoungTest() throws Exception {
		...
		Person person = new PersonTestDataBuilder().withId(id).withlastName("carrero").withAge(4).withName("mauro").build();
	...
	public void savePersonAdultTest() throws Exception {
		...
		int id = 10; 
		ObjectMapper mapper = new ObjectMapper();
		Person person = new PersonTestDataBuilder().withId(id).withlastName("contreras").withAge(56).withName("franklin").build();
		RequestBuilder requestBuilder = MockMvcRequestBuilders.post("/people")
				.content(mapper.writeValueAsString(person)).contentType(MediaType.APPLICATION_JSON);
	...
	public void getPeopleTest() throws Exception {
		...
		.andExpect(jsonPath("$", hasSize(2)))
				.andExpect(jsonPath("$[0].id", equalTo(1000)))
				.andExpect(jsonPath("$[1].id", equalTo(2000)));
	}
	...
	public void getPersonTest() throws Exception {
		...
		RequestBuilder requestBuilder = MockMvcRequestBuilders.get("/people/5000").contentType(MediaType.APPLICATION_JSON);
		// act
		mockMvc.perform(requestBuilder).andDo(print())
		.andExpect(status().isOk())
				.andExpect(jsonPath("name", equalTo("pedro")))
				.andExpect(jsonPath("lastName", equalTo("gonzalez")));
	}
	...
	public void getPersonNotFoundTest() throws Exception {
		...
		RequestBuilder requestBuilder = MockMvcRequestBuilders.get("/people/909855").contentType(MediaType.APPLICATION_JSON);
		// act
